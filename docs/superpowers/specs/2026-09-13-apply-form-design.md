...
- 卡片一「批次匯入」：
  ...
  - **欄位對應**：第一個非空行若含「科號」視為標題列，依標題名稱對應（同義詞：`科號`；`中文課名`／`課名`／`課程名稱`；`英文課名`；`上課時間`／`時間`；`教師`／`授課教師`），順序無關，英文課名可缺；無標題列時依校方檔案順序 `科號、中文課名、英文課名、上課時間、教師`；標題列缺「科號」「中文課名」「上課時間」「教師」任一 → 整批錯誤「標題列缺少欄位：…」
