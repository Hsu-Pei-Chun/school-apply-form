...
日期：2026-09-15（v4.1：依校方真實課程檔修正——科號含空格、匯入欄位依標題對應、英文課名、CSV 引號）
...
### v4.1 依校方真實課程檔（11510xclass1.xlsx，54 科）的修正

| 實況 | 修正 |
|---|---|
| 科號 15 碼**含空格補位**（例 `11510AIA 500700`、`11510CS  110400`），54 筆中 38 筆含空格 | 科號規則改為「15 碼，每碼為英數或空格」；條碼與掃描解析同步允許空格 |
| 欄位為 **科號、中文課名、英文課名、上課時間、教師**（5 欄） | 匯入依標題列名稱對應欄位（順序無關）；無標題時預設此 5 欄順序；新增 `courses.name_en` 存英文課名 |
| 教師欄含逗號（`台大李宗穎,周瑞賢`） | 逗號模式支援 RFC 4180 引號（`"…"` 內的逗號不切） |
...
courses                    X-Class 課程主檔
  code        TEXT PK      科號，固定 15 碼，每碼為英數或空格（CHECK length = 15；空格為校方補位，需原樣保留）
  name        TEXT         課程名稱（中文）
  name_en     TEXT         英文課名，可為空字串
  teacher     TEXT         授課教師
...
- v2 → v3、v3 → v4、v4 → v4.1 皆為破壞性變更：重產 migration `0000`，重 seed。
...
- 卡片一「批次匯入」：
  - `textarea` 貼上 Excel 內容，每行一科；分隔符自動偵測（Tab 優先，其次逗號；逗號模式支援 `"…"` 引號包住含逗號的欄位）；也可上傳 `.csv`/`.txt` 檔，內容讀進同一 textarea
  - **欄位對應**：第一個非空行若有儲存格與「科號」完全相符（trim 後）視為標題列，依標題名稱對應（同義詞：`科號`；`中文課名`／`課名`／`課程名稱`；`英文課名`；`上課時間`／`時間`；`教師`／`授課教師`），順序無關，英文課名可缺；無標題列時依校方檔案順序 `科號、中文課名、英文課名、上課時間、教師`；標題列缺「科號」「中文課名」「上課時間」「教師」任一 → 整批錯誤「標題列缺少欄位：…」
...
| 重複掃描 | 黃卡，保留第一次收件時間 |
| 課程科號非 15 碼（英數或空格）/ 重複新增 | server 拒絕，表單顯示錯誤 |
| 匯入標題列缺必要欄位 | 整批錯誤，訊息列出缺少的欄位名 |
...
- 100 課程：科號 15 碼 = `11510` + 系所 4 碼（`EECS`/`MATH`/`PHYS`/`CHEM`/`ECON`/`CHIN`/`LANG`/`LIFE`/`MSE0`/`CS  `，含空格補位者模擬校方格式）+ 6 碼課號（`200101` 起）；`name_en` 給簡單英文名；名稱「微積分 / 普通物理 …」+ 編號；教師「姓氏 + 教授」；上課時間輪流取 `M1M2` / `T3T4` / `W5W6` / `R7R8` / `F1F2` / `M3M4R3R4`
- 不預先產生申請單
...
- 批次匯入：Tab / 逗號分隔皆可；標題列依名稱對應且順序無關；無標題走預設 5 欄；標題缺必要欄位整批錯誤；空行跳過；欄位不足、科號格式、空白欄位、同批重複各自標紅；已存在標黃且不寫入；plan 有紅字時 apply 拒絕；apply 為 transaction 且回傳筆數；BOM 與 CRLF 正常處理；逗號模式引號內逗號不切；**校方 54 科真實資料（tests/fixtures/xclass-11510.tsv）全部為 add**
- 科號含空格：`createCourse` 接受、條碼 24 碼含空格可產生、`receiveByInput` 對含空格的 24 碼可對到申請單
- 條碼 SVG：內容 24 碼可產生
