...
學生登入後手填 1～5 門一般課程（科號、課名、上課時間、任課教師），選一門 X-Class 課程（科號 15 碼），產生右上角含 24 碼條碼的申請表（含上課時間），列印後由課務組掃碼收件。
...
- `/admin/courses`：課程管理（新增／停用／批次匯入：貼上 Excel（第一行為標題：科號、中文課名、英文課名、上課時間、教師，順序不限）或上傳 UTF-8 CSV，預覽後整批寫入；科號已存在會略過。科號 15 碼可含空格補位（如 `11510AIA 500700`），系統會原樣保留。）
- `/admin/scan`：行政掃描收件。條碼機請保留空格（勿開啟 suppress space 之類設定），科號含空格補位時條碼內也有空格；若掃描結果為「查無此申請單」，可改為手動輸入申請表頁尾的申請單號（如 A000001）。

## 假資料

`npm run seed` 會產生：
- 學生：`113000001` 到 `113002000`（2000 筆）
- 課程：科號 15 碼格式（如 `11510XXXX2001NN`），含上課時間

## 接 SSO

若需整合校內 SSO 系統，修改 `lib/auth.ts` 檔案中的登入邏輯即可，資料庫 schema 無須變更。正式環境的 `sid` cookie 需改為簽章或不透明 session id，不可再直接存學號。
